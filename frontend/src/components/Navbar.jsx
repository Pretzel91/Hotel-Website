...
const Navbar = () => {
    const navLinks = [
        { name: 'Home', path: '/' },
        { name: 'Tours', path: '/id' },
        { name: 'Boutique Hotel', path: '/rooms' },
    ...
    return (
            <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                {/* Logo */}
                <Link to='/'>
                    <img src={Logo} alt="Logo" className={`h-16 ${isScrolled && "invert opacity-80"}`} />
                </Link>
                ...
                <div className="hidden md:flex items-center gap-4">
                    <CiSearch className={`${isScrolled && 'invert'} h-7 transition-all duration-500`}/>
                    {user ? 
                    (<UserButton>
                ...
                {user && <UserButton>
                    ...
                    <IoMenu onClick={() => setIsMenuOpen(!isMenuOpen)} className={`${isScrolled && "invert"} h-4`}/>
                </div>

                {/* Mobile Menu */}
                <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    <button className="absolute top-4 right-4" onClick={() => setIsMenuOpen(false)}>
                        <MdClose alt="close-menu" className="h-6.5"/>
                    </button>
