  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        {/* <div>
            {room.amenities.map((item, index)=>(
                <div key={index} className='flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-100'>
                    <img src={faciltyIcons[item]} alt={item} className='w-5 h-5' />
        ...
        <form className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
            ...
            <button type='submit' className='bg-blue-700 hover:bg-blue-600 active:scale-95 transition-all text-white rounded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer'>
                Check Availability
            </button>
        </form>
        ...
        <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
            <p>Guests will be allocated on the ground floor according to availability. You get a comfortable two bedroom apartment has a true city feeling. The price quoted is for two guest, at the guest slot please mark the number of guests to get the exact price for groups. The guests will be allocated ground floor according to availabilty. You get the comfortable two bedroom apartment that has a true city feeling.</p>
        </div>

        <div className='flex flex-col items-start gap-4'>
            <div className='flex gap-4'>
                <img src={Logo} alt="" className='h-30 w-24 md:h-24 md:w-20 rounded-full' />
                <div>
            ...
            <button className='px-6 py-2.5 mt-4 rounded text-white bg-blue-600 hover:bg-blue-500 transition-all cursor-pointer'>Contact Now</button>
        </div>
    </div>
  )
}
